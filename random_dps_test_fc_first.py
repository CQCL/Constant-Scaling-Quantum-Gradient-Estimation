...
def get_losses(differentiator, n_features, epochs):
    ...
    for _ in trange(N_ITERATIONS, desc="Iterations", leave=False, position=1):
        ...
        # Init model
        input_layer = torch.nn.Linear(n_features, n_features)
        ...
        model = torch.nn.Sequential(input_layer, qlayer, sig).to(GPU_DEVICE)
        ...
        acc = Accuracy('binary').to(GPU_DEVICE)
...
for ps_eps, n_feat in tqdm(zip(PS_EPOCHS, N_FEATURES), desc="Features", leave=False, position=0):
    ...
    file_template = "data_initial_exp_fc_first/fc_{}_{}_niter{}_nfeatures{}_nlayers{}_batchsize{}_nepochs{}_lr{}.npy"
